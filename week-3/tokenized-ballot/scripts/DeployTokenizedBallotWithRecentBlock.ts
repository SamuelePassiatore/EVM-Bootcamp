...
async function main() {
    console.log("Deploying TokenizedBallot contract with recent target block...");
    ...
    // Get a block number slightly in the past
    const currentBlock = await publicClient.getBlockNumber();
    // Use a block that's only 10 blocks in the past
    const targetBlockNumber = Number(currentBlock) - 10;
